...
def create_ocp_solver_description() -> AcadosOcp:
    ...
    nx = model.x.size()[0]
    nu = model.u.size()[0]
    ny = nx + nu #5 states + 2 controls = 7
    ...
    print("size of W", ocp.cost.W.shape)
    ...
    print("size of Vx", ocp.cost.Vx.shape)
    print("size of Vu", ocp.cost.Vu.shape)

    ocp.cost.Vx_e = np.eye(nx)
    print("size of Vx_e", ocp.cost.Vx_e.shape)

    ocp.cost.yref = np.zeros((ny,))
    ocp.cost.yref_e = np.zeros((ny_e,))
    print("size of yref", ocp.cost.yref.shape)
    print("size of yref_e", ocp.cost.yref_e.shape)
    ...
    print("size of lbu", ocp.constraints.lbu.shape)
    print("size of ubu", ocp.constraints.ubu.shape)
    print("size of idxbu", ocp.constraints.idxbu.shape)

    ocp.constraints.x0 = X0
    print("x0", ocp.constraints.x0)
...
def closed_loop_simulation():
    ...
    acados_ocp_solver.print_statistics()
    # acados_ocp_solver.get_from_qp_in("x", 0, simX[0, :])
    #get constraints
    print("lbx constraints", acados_ocp_solver.get_from_qp_in(0, "lbx"))
    print("lbu constraints", acados_ocp_solver.get_from_qp_in(5, "lbu"))
